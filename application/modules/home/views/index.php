...
<script>
	...
	*/
	...
	/* =========================================================
| GENERATE MATERIAL PDF (FIXED CONSISTENCY)
========================================================= */

	function generateMaterialPDF(materialCode) {
		Swal.fire({
		...
		$.ajax({
			...
			success: function(res) {
				...
				if (res.status !== 'success' || !res.data) {
					Swal.fire('Error', res.message || 'Material not found', 'error');
					return;
				}
				...
				IMAGE
				===================================================== */
				let materialImage = base + 'assets/images/No_Image_Available.jpg';
				if (m.images && m.images.length > 0) {
					let cover = m.images.find(x => x.is_cover == 1);
					if (!cover) {
						cover = m.images[0];
					}
					materialImage = base + 'uploads/material_assets/images/' + cover.file_name;
				}
				...
				let qrImage = '';
				if (m.qr_code_file) {
					qrImage = base + 'uploads/qrcode/' + m.qr_code_file;
				}

				let allImages = [];
				if (m.images && m.images.length > 0) {
					allImages = m.images.map(img => base + 'uploads/material_assets/images/' + img.file_name);
				}

				let locations = '-';
				if (m.locations && m.locations.length > 0) {
					locations = m.locations.map(x => x.location_name).join(', ');
				}

				let models = '-';
				if (m.models && m.models.length > 0) {
					models = m.models.map(x => x.model_number).join(', ');
				}

				let classifications = '-';
				if (m.classifications && m.classifications.length > 0) {
					classifications = m.classifications.map(x => x.classification_name).join(', ');
				}

				let coverImage = materialImage;
				...
				if (m.images && m.images.length > 0) {
					...
					additionalImages = m.images.filter(x => x.file_name !== cover.file_name);
				}
				...
				/* =====================================================
				PAGE 1 (Fix: Penyeragaman posisi footer ke bottom: 60px)
				===================================================== */
				let firstPage = `
            <div style="
                width: 100%;
            ...
            ">
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 5px;">
                    <tr>
                        <td style="text-align: left; vertical-align: bottom;">
                            <div style="font-size: 24px; font-weight: 700; color: #0f4c75; letter-spacing: 1px;">
                                <i class="bi bi-box-seam-fill"></i> MATERIAL LIBRARY
                            </div>
                        </td>
                        <td style="text-align: right; vertical-align: bottom;">
                            <div style="font-weight: 700; font-size: 18px; text-transform: uppercase; color: #333;">
                                ${m.material_name || '-'}
                            </div>
                        </td>
                    </tr>
                </table>
                <div style="background-color: #0f4c75; height: 4px; width: 100%; margin-bottom: 20px;"></div>
                ...
                <div style="width: 100%;">
                    <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 11px 5px; color: #555; font-weight: bold; width: 22%;">MLM ID</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #000; width: 28%;">: ${m.mlm_ref_no || '-'}</td>
                            <td style="padding: 11px 5px; color: #555; font-weight: bold; width: 25%;">Sample Leadtime</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #000; width: 25%;">: ${m.sample_leadtime || '-'} Days</td>
                        </tr>
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">Supplier Name</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #000;">: ${m.supplier_name || '-'}</td>
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">Production Leadtime</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #000;">: ${m.production_leadtime || '-'} Days</td>
                        </tr>
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">Material Type</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #000;">: ${m.material_type_name || '-'}</td>
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">Production Location</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #000;">: ${locations}</td>
                        </tr>
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">Price</td>
                            <td style="padding: 11px 5px; font-weight: bold; color: #d32f2f;">: $${m.standard_price || '-'}</td>
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">Lifecycle State</td>
                            <td style="padding: 11px 5px;">
                                : <span style="background-color: ${badgeColor}; color: white; padding: 2px 8px; border-radius: 3px; font-size: 11px; font-weight: bold;">
                                    ${m.lifecycle_name || '-'}
                                </span>
                            </td>
                        </tr>
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 11px 5px; color: #555; font-weight: bold;">First Season</td>
                            <td colspan="3" style="padding: 11px 5px; font-weight: bold; color: #000;">: ${m.season_name || '-'} ${m.season_year || ''}</td>
                        </tr>
                    </table>
                </div>

                <div style="margin-top: 20px;">
                    <table style="width: 100%; border-collapse: collapse; font-size: 13px; border: 1px solid #0f4c75;">
                        <tr style="background-color: #f9f9f9;">
                            <td style="width: 33.33%; padding: 12px; text-align: center; border-right: 1px solid #0f4c75;">
                                <span style="color: #666; font-weight: bold; text-transform: uppercase; font-size: 11px;">Weight</span><br>
                                <span style="font-size: 16px; font-weight: bold; color: #000; display: inline-block; margin-top: 4px;">
                                    ${m.weight || '-'} ${m.weight_uom_name || 'G/SQM'}
                                </span>
                            </td>
                            <td style="width: 33.33%; padding: 12px; text-align: center; border-right: 1px solid #0f4c75;">
                                <span style="color: #666; font-weight: bold; text-transform: uppercase; font-size: 11px;">Thickness</span><br>
                                <span style="font-size: 16px; font-weight: bold; color: #000; display: inline-block; margin-top: 4px;">
                                    ${m.thickness || '-'} mm
                                </span>
                            </td>
                            <td style="width: 33.33%; padding: 12px; text-align: center;">
                                <span style="color: #666; font-weight: bold; text-transform: uppercase; font-size: 11px;">Width</span><br>
                                <span style="font-size: 16px; font-weight: bold; color: #000; display: inline-block; margin-top: 4px;">
                                    ${m.width || '-'} ${m.width_uom_name || 'INCH'}
                                </span>
                            </td>
                        </tr>
                    </table>
                ...
                <div style="position: absolute; bottom: 60px; left: 25px; right: 25px; background: #0f4c75; color: white; padding: 12px 25px; box-sizing: border-box;">
                    <table style="width: 100%; border-collapse: collapse;">
                        <tr>
                            <td style="vertical-align: middle; text-align: left;"></td>
                            <td style="vertical-align: middle; text-align: right;">
                                <div style="display: inline-flex; align-items: center; justify-content: flex-end; gap: 12px;">
                                    <img src="${base}assets/images/3.jpeg" style="height: 22px; width: auto; display: inline-block; vertical-align: middle;" onerror="this.style.display='none';">
                                    <span style="font-size: 22px; font-weight: bold; letter-spacing: 5px; display: inline-block; vertical-align: middle; line-height: 22px;">PARKLAND</span>
                                </div>
                            </td>
                        </tr>
                    </table>
            ...
            `;
                ...
                <div style="
                    ...
                    position: relative;
                ">
                    <div style="text-align: center; color: #0f4c75; font-size: 24px; font-weight: 700; margin-bottom: 15px;">
                        MATERIAL IMAGES
                    </div>
                ...
                `;
					...
					for (let i = 0; i < 2; i++) {
                        ...
                        <div style="
                            ...
                            height: 400px; 
                        ">
                            <img src="${base}uploads/material_assets/images/${img.file_name}" style="
                                width: 100%;
                                height: 100%;
                                object-fit: contain;
                                display: block;
                    ...
                    <div style="position: absolute; bottom: 60px; left: 25px; right: 25px; background: #0f4c75; color: white; padding: 12px 25px; box-sizing: border-box;">
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                <td style="vertical-align: middle; text-align: right;">
                                    <div style="display: inline-flex; align-items: center; justify-content: flex-end; gap: 12px;">
                                        <img src="${base}assets/images/3.jpeg" style="height: 30px; width: auto;" onerror="this.style.display='none';">
                                        <span style="font-size: 20px; font-weight: bold; letter-spacing: 5px;">PARKLAND</span>
                                    </div>
                                </td>
                            </tr>
                        </table>
                ...
                ">
                    ...
                    <div style="
                        ...
                        height: 820px;
                    ">
                        <img src="${base}uploads/material_assets/images/${additionalImages[2].file_name}" style="
                            width: 100%;
                            height: 100%;
                            object-fit: contain;
                            display: block;
                    ...
                    <div style="position: absolute; bottom: 60px; left: 25px; right: 25px; background: #0f4c75; color: white; padding: 12px 25px; box-sizing: border-box;">
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                <td style="vertical-align: middle; text-align: right;">
                                    <div style="display: inline-flex; align-items: center; justify-content: flex-end; gap: 12px;">
                                        <img src="${base}assets/images/3.jpeg" style="height: 30px; width: auto;">
                                        <span style="font-size: 20px; font-weight: bold; letter-spacing: 5px;">PARKLAND</span>
                                    </div>
                                </td>
                            </tr>
				...
				let html = `${firstPage}${imageGallery}`;
				let element = document.createElement('div');
				element.innerHTML = html;

				if (typeof html2pdf === 'undefined') {
					let script = document.createElement('script');
					script.src = `${base}assets/js/html2pdf.bundle.min.js`;
					script.onload = function() {
						generatePDFContent(element, m.material_code);
					};
					script.onerror = function() {
						Swal.fire('Error', 'Failed to load PDF library', 'error');
					};
			...
			error: function(xhr) {
				...
				Swal.fire('Error', 'Failed to get material detail: ' + (xhr.responseJSON?.message || 'Unknown error'), 'error');
			}
		});
	}

	/* =========================================================
	| GENERATE PDF CONTENT (FIXED CONFIGURATION)
	========================================================= */
	function generatePDFContent(element, materialCode) {
		try {
			html2pdf()
				.set({
					// FIX 1: Ubah margin global ke [0,0,0,0] agar mengikuti dimensi CSS A4 murni (1120px)
					margin: [0, 0, 0, 0],
					filename: 'Material_' + materialCode + '.pdf',
					image: {
						type: 'jpeg',
						quality: 0.98
					},
					html2canvas: {
						scale: 2,
						useCORS: true,
						allowTaint: false, // FIX 2: Ubah ke false agar tidak bentrok dengan aturan CORS gambar lokal
			...
			Swal.fire('Error', 'Error generating PDF: ' + error.message, 'error');
		}
	}
