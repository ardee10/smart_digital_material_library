...
<div class="container-fluid">
	<div class="row mb-3">
		<div class="col-md-4 mb-3">
			...
			<button type="button"
				...
				data-bs-toggle="modal"
				data-bs-target="#modalImportLocation">
		...
		<div class="table-responsive">
			<table class="table custom-table align-middle" style="width:100%" id="tblLocation">
						...
						<tr>
							...
							<td>
								...
								<div class="d-flex gap-2 justify-content-center">
									...
									<a href="<?php echo base_url('admin/deleteLocation/' . $data->id); ?>"
	...
	<div class="modal fade" id="modalAddLocation" tabindex="-1" role="dialog" aria-labelledby="modalAddLocationLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="modalAddLocationLabel">Add Location</h5>
	...
	<div class="modal fade" id="modalImportLocation" tabindex="-1" role="dialog" aria-labelledby="modalImportLocationLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="modalImportLocationLabel">Import Location</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<form id="formImportLocation">
					<div class="modal-body">
						<div class="mb-3">
				...
				<div class="modal-body justify-content-center">
					<a href="<?= base_url('assets/template_import/import_location.xlsx') ?>" class="btn btn-outline-success btn-sm" download>
						<i class="bi bi-download"></i>
						Download
					</a>
					<p class="text-muted mt-2">Download format excel for import location data</p>
				</div>
	...
	<script>
		...
		function editLocation(el) {
			...
			$('#modalAddLocationLabel').text('Edit Location');
			$('#btnSaveLocation').text('Update');

			// Call ajax to get data by ID
			$.ajax({
				url: base + 'Admin/getLocById/' + id,
				type: 'GET',
				dataType: 'json',
		...
		$('#btnSaveLocation').on('click', function() {
			...
			let url = id ?
				base + 'Admin/updateLocation/' + id :
				base + 'Admin/saveLocation';
			$.ajax({
				url: url,
				type: 'POST',
		...
		function resetFormData() {
			...
			$('#modalAddLocationLabel').text('Add Location');
			$('#btnSaveLocation').text('Save');
		}
		// Reset form when modal is hidden
		$('#modalAddLocation').on('hidden.bs.modal', function() {
			resetFormData();
		});

		//Import Location
		$('#formImportLocation').on('submit', function(e) {
			e.preventDefault();
			let formData = new FormData(this);
			$.ajax({
				url: base + 'Admin/importLocation',
				type: 'POST',
				data: formData,
				contentType: false,
				processData: false,
				...
				success: function(res) {
					if (res.status == 'success') {
						// CLOSE MODAL
						$('#modalImportLocation').modal('hide');
						// RESET FORM
						$('#formImportLocation')[0].reset();
						// Load Model Number
						Swal.fire({
							icon: 'success',
							title: 'Success',
